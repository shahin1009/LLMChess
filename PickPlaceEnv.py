...
class PickPlaceEnv():
  ...
  def reset(self, config):
    ...
    for obj_name in obj_names:
      ...
      if ("block" in obj_name) or \
        ("bowl" in obj_name) or \
        ("Pawn" in obj_name) or \
        ("Queen" in obj_name) or \
        ("King" in obj_name) or \
        ("Knight" in obj_name) or \
        ("Rook" in obj_name) or \
        ("Bishop" in obj_name) or \
        ("sphere" in obj_name) or \
        ...
        if object_type == "block":
        ...
        elif object_type == "Bishop":
          object_position[2] = 0
          object_id = pybullet.loadURDF("bowl/Bishop.urdf", object_position, useFixedBase=1)
        elif object_type == "Pawn":
          object_position[2] = 0
          object_id = pybullet.loadURDF("bowl/Pawn.urdf", object_position, useFixedBase=1)
        elif object_type == "King":
          object_position[2] = 0
          object_id = pybullet.loadURDF("bowl/King.urdf", object_position, useFixedBase=1)
        elif object_type == "Knight":
          object_position[2] = 0
          object_id = pybullet.loadURDF("bowl/Knight.urdf", object_position, useFixedBase=1)
        elif object_type == "Rook":
          object_position[2] = 0
          object_id = pybullet.loadURDF("bowl/Rook.urdf", object_position, useFixedBase=1)
        elif object_type == "Queen":
          print(obj_name) # Debug
          object_position[2] = 0
          object_id = pybullet.loadURDF("bowl/Queen.urdf", object_position, useFixedBase=1)
          print("Body Info:", pybullet.getBodyInfo(object_id))
          ...
          object_id = pybullet.createMultiBody(0.01, object_shape, object_visual, basePosition=object_position)
          # Adjust Z position if needed, spheres are centered at their position
          object_position[2] = radius # Place it so bottom touches the plane

        elif object_type == "cylinder":
          radius = 0.02 # Example radius
          height = 0.05 # Example height (use 'length' for visual shape)
          object_shape = pybullet.createCollisionShape(pybullet.GEOM_CYLINDER, radius=radius, height=height)
          object_visual = pybullet.createVisualShape(pybullet.GEOM_CYLINDER, radius=radius, length=height) # Note: length here
          object_id = pybullet.createMultiBody(0.01, object_shape, object_visual, basePosition=object_position)
          # Adjust Z position for cylinders, they are centered height-wise
          object_position[2] = height / 2 # Place it so bottom touches the plane
